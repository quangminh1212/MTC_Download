...
def download_decrypt_write(book_dir: Path, book_name: str, idx: int, total: int, ch: dict):
    d = MTCDownloader()  # session per worker for thread safety
    cid = ch.get('id')
    ...
    if not content:
        return idx, False, f'MISS content chapter_id={cid} file={fname}'
    ...
    return idx, True, f'OK {idx}/{total} decrypted={decrypted} file={fname}'
...
def main() -> int:
    ...
    # Folder must contain chapters only: remove metadata if any.
    for junk in ['info.json', '_decrypt_report.txt']:
        p = book_dir / junk
        if p.exists():
            p.unlink()
    print(f'final_ok={ok}')
