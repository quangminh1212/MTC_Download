...
for b in completed[:300]:
    ...
    if not chapters:
        continue
    # sample first, middle, last for lock check
    ...
    for ch in uniq:
        ...
        if row['is_locked'] not in (0, None):
            ok = False
        if row['content_len'] < 2000:
            ok = False
...
out = Path(r'C:\Dev\MTC_Download\logs\latest_free_completed_candidate.json')
out.parent.mkdir(parents=True, exist_ok=True)
out.write_text(json.dumps(candidates, ensure_ascii=False, indent=2), encoding='utf-8')
print(str(out))
if candidates:
    print(json.dumps(candidates[0], ensure_ascii=True))
