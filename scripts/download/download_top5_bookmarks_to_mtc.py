...
from download_completed_to_mtc import chapter_filename as mtc_chapter_filename, safe_book_dir_name
from mtc_status_utils import cleanup_root_completed_dirs, is_completed_status, is_ongoing_status, move_completed_book_folder
...
def select_unfinished_books(manifest_books, top_n: int):
    selected = []
    skipped = []
    for book in manifest_books:
        ...
        if not is_ongoing_status(book):
            skipped.append({'id': book.get('id'), 'name': book.get('name'), 'reason': book.get('status_name') or book.get('status') or 'unknown_status'})
            continue
...
def chapter_filename(chapter: dict) -> str:
    return mtc_chapter_filename({'index': chapter['index'], 'name': chapter['name']}, chapter['index'])
...
def reconcile_existing_chapter_files(folder: Path, chapters):
    ...
    for index, paths in by_index.items():
        ...
        if chapter is None:
            continue
        canonical = folder / chapter_filename(chapter)
...
def collect_existing_ids(folder: Path, chapters):
    ids = set()
    for chapter in chapters:
        path = folder / chapter_filename(chapter)
...
def fetch_chapter_text(token: str, chapter, folder: Path):
    ...
    index = chapter['index']
    title = chapter['name']
    out = folder / chapter_filename(chapter)
...
def _git_status_names(folder: Path):
    proc = subprocess.run(
        ['git', '-C', str(ROOT), 'status', '--porcelain=v1', '--', str(folder)],
...
def maybe_commit_verified_book(folder: Path, book_name: str, verified_complete: bool):
    if not verified_complete:
        return {'status': 'not_complete'}
    status_lines = _git_status_names(folder)
    if not status_lines:
        return {'status': 'nothing_to_commit'}
    for attempt in range(3):
        add = subprocess.run(
            ['git', '-C', str(ROOT), 'add', '-A', '--', str(folder)],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            check=False,
            encoding='utf-8',
        ...
        add_error = add.stderr.strip()
        if add.returncode != 0:
            if 'index.lock' in add_error and _clear_stale_git_lock():
                time.sleep(1)
                continue
            return {'status': 'add_failed', 'error': add_error}
        staged = subprocess.run(
            ['git', '-C', str(ROOT), 'diff', '--cached', '--name-only', '--', str(folder)],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            check=False,
            encoding='utf-8',
        ...
        commit = subprocess.run(
            ['git', '-C', str(ROOT), '-c', 'commit.gpgsign=false', '-c', 'core.hooksPath=NUL', 'commit', '--no-verify', '-m', book_name, '--', str(folder)],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            check=False,
            encoding='utf-8',
        ...
        commit_error = commit.stderr.strip()
        if commit.returncode == 0:
            return {'status': 'committed', 'staged_count': len(staged_files), 'message': book_name}
        if 'index.lock' in commit_error and _clear_stale_git_lock():
            time.sleep(1)
            continue
        return {'status': 'commit_failed', 'error': commit_error, 'staged_count': len(staged_files)}
...
def parse_args(argv):
    ...
    if not email or not password:
        raise SystemExit('usage: python download_top5_bookmarks_to_mtc.py <email> <password> [top_n] [delay] [workers] [verify_passes]')
    return {
        'email': email,
        'password': password,
        'top_n': int(argv[arg_offset]) if len(argv) > arg_offset else int(os.environ.get('MTC_TOP_N', '5')),
...
def main():
    ...
    for book_index, book in enumerate(books, 1):
        ...
        if is_completed_status(book_detail):
            if folder.exists():
                action = move_completed_book_folder(ROOT, folder)
                print(f"  skip completed; {action['status']} -> {action['destination']}", flush=True)
            else:
                print('  skip completed; no root folder created', flush=True)
            continue
        ...
        commit_result = maybe_commit_verified_book(folder, name, result['ok'] == result['expected'] and not result['missing_ids'])
        entry = {
            'id': book_id,
            'name': name,
            'folder': str(folder),
        ...
        save_report(ordered)
        print(f"  final ok={entry['ok']}/{entry['expected']} duplicates={len(entry['duplicates'])} missing={len(entry['missing_ids'])}", flush=True)
        if commit_result.get('status') == 'committed':
            print(f"  committed {name}", flush=True)
        elif commit_result.get('status') not in {'not_complete', 'nothing_to_commit'}:
            print(f"  commit={commit_result.get('status')}", flush=True)
