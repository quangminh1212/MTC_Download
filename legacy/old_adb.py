"""adb.py ΓÇô ADB controller for BlueStacks emulator (speed-optimized)."""
...
class AdbController:
    ...
    # ΓöÇΓöÇ Low-level ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    # ΓöÇΓöÇ Find ADB ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    @staticmethod
    ...
    # ΓöÇΓöÇ Device management ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    # ΓöÇΓöÇ APK ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    def install_apk(self, apk_path: Path = APK_PATH,
                    log_fn: Callable[[str], None] = print) -> bool:
        if not apk_path.exists():
            log_fn(f"APK kh├┤ng t├¼m thß║Ñy: {apk_path}")
            return False
        log_fn(f"─Éang c├ái {apk_path.name}...")
        out, err = self._cmd("install", "-r", "-d", str(apk_path),
                             timeout=INSTALL_TIMEOUT)
        ok = "success" in (out + err).lower()
        log_fn("Γ£ö C├ái xong" if ok else f"Γ£û Lß╗ùi: {(out+err)[:300]}")
        return ok
    ...
    # ΓöÇΓöÇ App control ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    # ΓöÇΓöÇ Accessibility ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    def enable_accessibility(self, log_fn: Callable[[str], None] = print) -> bool:
        for svc in ACCESSIBILITY_SERVICES:
            ...
            if current and svc.split("/")[0] in current:
                ...
                return True
        self.sh("settings", "put", "secure", "accessibility_enabled", "1")
        log_fn("ΓÜá Kh├┤ng t├¼m thß║Ñy accessibility service chuß║⌐n")
        return False
    ...
    # ΓöÇΓöÇ Screen ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    def _main_tabs_visible(self, xml_str: str = "") -> bool:
        ...
        return all(label in texts for label in ("Tß╗º Truyß╗çn", "Kh├ím Ph├í", "Xß║┐p Hß║íng", "T├ái Khoß║ún"))
    ...
    def _find_bottom_tab_center(self, tab_index: int, xml_str: str = "") -> Optional[tuple]:
        ...
        marker = f"Tab {tab_index} trong tß╗òng sß╗æ 4"
        for node in root.iter():
    ...
    def ensure_main_tabs(self, log_fn: Callable[[str], None] = print) -> bool:
        ...
        if not pkg:
            log_fn("ΓÜá Kh├┤ng t├¼m thß║Ñy package MTC ─æß╗â mß╗ƒ app")
            return False
        ...
        for attempt in range(2):
            ...
            if self._main_tabs_visible():
                return True
            log_fn(f"ΓÜá Ch╞░a vß╗ü ─æ╞░ß╗úc m├án h├¼nh ch├¡nh (l╞░ß╗út {attempt + 1})")
        return False
    ...
    def open_explore_tab(self, log_fn: Callable[[str], None] = print) -> bool:
        ...
        if not center:
            log_fn("ΓÜá Kh├┤ng t├¼m thß║Ñy tab Kh├ím Ph├í")
            return False
    ...
    def open_library_tab(self, log_fn: Callable[[str], None] = print) -> bool:
        ...
        if not center:
            log_fn("ΓÜá Kh├┤ng t├¼m thß║Ñy tab Tß╗º Truyß╗çn")
            return False
        ...
        xml = self.dump_ui()
        history_center = self._find_clickable_text_center("Lß╗ïch sß╗¡", xml_str=xml)
        if history_center:
        ...
        return any(text == "Tß╗º Truyß╗çn" for text in texts)
    ...
    def open_search_screen(self, log_fn: Callable[[str], None] = print) -> bool:
        ...
        if len(blank_buttons) < 1:
            log_fn("ΓÜá Kh├┤ng t├¼m thß║Ñy n├║t mß╗ƒ m├án h├¼nh t├¼m kiß║┐m")
            return False

        rect = blank_buttons[0][0]
        self.tap((rect[0] + rect[2]) // 2, (rect[1] + rect[3]) // 2)
        time.sleep(0.8)
        return bool(self._parse_bounds(self.dump_ui(), "T├¼m", exact=False))
    ...
    # ΓöÇΓöÇ UIAutomator (speed-optimized) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    def dump_ui(self) -> str:
        """Dump UI XML ΓÇô shell combined command first (fastest on BlueStacks)."""
        self.ensure_device()
    ...
    def _extract_reader_payload(xml_str: str) -> Optional[Dict]:
        ...
        title = lines[0]
        match = re.search(r"Ch╞░╞íng\s+(\d+)", title, re.IGNORECASE)
        body_lines = lines[1:]
    ...
    def _reader_menu_visible(self, xml_str: str) -> bool:
        ...
        return "d.s ch╞░╞íng" in lookup and "tß║úi lß║íi nß╗Öi dung" in lookup
    ...
    def reader_next_chapter(self, log_fn: Callable[[str], None] = print) -> bool:
        ...
        if not self._reader_menu_visible(overlay_xml):
            log_fn("  Fast next bß╗Å qua: kh├┤ng bß║¡t ─æ╞░ß╗úc menu reader")
            return False
        ...
        if before_idx is not None and after_idx == before_idx + 1:
            log_fn("  Sang ch╞░╞íng tiß║┐p theo (fast)")
            return True
        if before and after and before.get("title") != after.get("title"):
            log_fn("  Sang ch╞░╞íng tiß║┐p theo (fast)")
            return True

        log_fn("  Fast next thß║Ñt bß║íi, sß║╜ fallback sang mß╗ƒ danh s├ích ch╞░╞íng")
        return False

    def _dump_ui_fast(self) -> str:
        """dump_ui without ensure_device() ΓÇô for turbo loop where device is known."""
        out, err = self._cmd(
            "shell",
            "uiautomator dump --compressed /sdcard/_ui.xml >/dev/null 2>&1; cat /sdcard/_ui.xml",
            timeout=8,
    ...
    def turbo_advance_and_read(self, log_fn: Callable[[str], None] = print) -> Optional[Dict]:
        ...
        if "<hierarchy" in merged:
            ...
            if self._reader_menu_visible(xml):
                # Menu still visible ΓÇô close and re-dump
                self.tap(cx, cy)
                time.sleep(0.15)
    ...
    def _chapter_list_visible(texts: List[str]) -> bool:
        if any(text.startswith("Sß╗æ ch╞░╞íng") for text in texts):
            return True
        return any(text[:1].isdigit() and "\nCh╞░╞íng " in text for text in texts)
    ...
    def _find_visible_chapter_item_center(self, xml_str: str = "") -> Optional[tuple]:
        ...
        for node in root.iter():
            ...
            if not value or "Ch╞░╞íng " not in value:
                continue
            if not (value.startswith("Ch╞░╞íng ") or value[:1].isdigit()):
                continue
    ...
    def open_chapter_list(self, log_fn: Callable[[str], None] = print) -> bool:
        ...
        center = self._parse_bounds(xml, "D.S Ch╞░╞íng", exact=False)
        if center:
            self.tap(*center)
            time.sleep(0.25)
            xml = self.dump_ui()
        ...
        if payload:
            ...
            time.sleep(0.4)
            overlay_xml = self.dump_ui()
            center = self._parse_bounds(overlay_xml, "D.S Ch╞░╞íng", exact=False)
            if center:
                self.tap(*center)
                time.sleep(0.35)
            xml = self.dump_ui()
        ...
        log_fn("ΓÜá Kh├┤ng mß╗ƒ ─æ╞░ß╗úc danh s├ích ch╞░╞íng")
        return False
    ...
    def _find_chapter_center(self, xml_str: str, chapter_index: int) -> Optional[tuple]:
        ...
        # Use word boundary to avoid matching "Ch╞░╞íng 4" inside "Ch╞░╞íng 419"
        pattern = re.compile(rf"Ch╞░╞íng[-: ]*\s*{chapter_index}(?:\D|$)", re.IGNORECASE)
        for node in root.iter():
            text = _clean_ui_text(node.get("text", ""))
            desc = _clean_ui_text(node.get("content-desc", ""))
    ...
    # ΓöÇΓöÇ MTC Navigation (speed-optimized) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    def nav_to_book(self, book_name: str,
                    log_fn: Callable[[str], None] = print) -> bool:
        log_fn("Mß╗ƒ m├án h├¼nh t├¼m truyß╗çn...")
        if not self.open_search_screen(log_fn):
            return False

        dump = self.dump_ui()
        center = self._parse_bounds(dump, "T├¼m", exact=False)
        if not center:
            log_fn("ΓÜá Kh├┤ng t├¼m thß║Ñy ├┤ t├¼m kiß║┐m trong app")
            return False

        self.tap(*center)
        time.sleep(0.2)
        log_fn(f"T├¼m: {book_name}")
        ...
        # Flutter renders search results on canvas ΓÇö not in accessibility tree.
        # Fallback: tap the first grid cell by coordinate (top-left result).
        log_fn("Γå¬ Kß║┐t quß║ú kh├┤ng accessible, thß╗¡ tß╗ìa ─æß╗Ö l╞░ß╗¢i ─æß║ºu ti├¬n...")
        w, h = self.screen_size()
        # First grid item center: ~15% from left, ~10.5% from top (900├ù1600 measured)
        fx = int(w * 0.15)
        fy = int(h * 0.105)
        self.tap(fx, fy)
        time.sleep(NAV_DELAY)
        ...
        if meta.get("read_center"):
            log_fn(f"Γ£ô Mß╗ƒ s├ích tß║íi ({fx},{fy})")
            return True
        log_fn("Γ£ù Tß╗ìa ─æß╗Ö l╞░ß╗¢i kh├┤ng dß║½n ─æß║┐n trang chi tiß║┐t s├ích")
        return False

    def nav_to_chapter(self, chapter_index: int,
                       log_fn: Callable[[str], None] = print) -> bool:
        log_fn(f"─Éi ─æß║┐n ch╞░╞íng {chapter_index}...")
        ...
        for skip_idx in range(40):
            ...
            visible = []
            pattern = re.compile(r"Ch╞░╞íng[-: ]*\s*(\d+)", re.IGNORECASE)
    ...
    def tap_library_book(self, book_name: str,
                         log_fn: Callable[[str], None] = print) -> bool:
        """From Library screen, find and tap a book by name ΓåÆ novel detail page."""
        xml = self.dump_ui()
        texts = self.get_all_text(xml)
        if not any("Tß╗º Truyß╗çn" in t for t in texts):
        ...
        if not best or best_score < 0.5:
            return False
        log_fn(f"  Tap '{best['title']}' tß╗½ Tß╗º Truyß╗çn...")
        self.tap(*best["center"])
        time.sleep(NAV_DELAY)
        return True
    ...
    def _parse_library_book_item(raw_desc: str, bounds: str) -> Optional[Dict]:
        ...
        progress_line = next((line for line in lines[1:] if "─É├ú ─æß╗ìc" in line), "")
        match = re.search(r"─É├ú ─æß╗ìc\s*(\d+)\s*/\s*(\d+)", progress_line, re.IGNORECASE)
        if not match:
            return None
    ...
    def scan_visible_library_books(
        ...
        try:
            ...
            log_fn("ΓÜá Kh├┤ng ─æß╗ìc ─æ╞░ß╗úc UI dump ß╗ƒ Tß╗º Truyß╗çn")
            return []
    ...
    def scan_library_books(
        ...
        for round_idx in range(max_scrolls):
            ...
            log_fn(f"Qu├⌐t Tß╗º Truyß╗çn l╞░ß╗út {round_idx + 1}: +{new_count} truyß╗çn")
    ...
    def open_library_book(
        ...
        for round_idx in range(max_scrolls):
            ...
            if best and best_score >= 0.70:
                log_fn(f"Mß╗ƒ tß╗½ Tß╗º Truyß╗çn: {best['title']}")
                self.tap(*best["center"])
                time.sleep(1.1)
                return best
            ...
            log_fn(f"Qu├⌐t Tß╗º Truyß╗çn ─æß╗â mß╗ƒ truyß╗çn: l╞░ß╗út {round_idx + 1}")
            self.swipe_up()

        log_fn(f"ΓÜá Kh├┤ng t├¼m thß║Ñy truyß╗çn trong Tß╗º Truyß╗çn: {book_name}")
        return None

    def get_book_detail_meta(self, xml_str: str = "") -> Dict:
        xml = xml_str or self.dump_ui()
        texts = self.get_all_text(xml)
        status_text = next((text for text in texts if text.startswith("Ch╞░╞íng - ")), "")
        read_center = self._find_clickable_text_center("─Éß╗ìc truyß╗çn", exact=True, xml_str=xml)
        if not read_center:
            read_center = self._find_clickable_text_center("─Éß╗ìc", exact=True, xml_str=xml)
        return {
    ...
    def detail_status_is_completed(status_text: str) -> bool:
        ...
        if not lowered:
            return False
        if "c├▓n tiß║┐p" in lowered:
            return False
        return "ho├án th├ánh" in lowered
    ...
    def open_current_book_reader(self, log_fn: Callable[[str], None] = print) -> bool:
        ...
        if not center:
            log_fn("  ΓÜá Kh├┤ng t├¼m thß║Ñy n├║t ─Éß╗ìc truyß╗çn")
            return False
        ...
        if self._chapter_list_visible(texts):
            ...
            if not chapter_center:
                log_fn("  ΓÜá ─Éang ß╗ƒ D.S Ch╞░╞íng nh╞░ng kh├┤ng t├¼m thß║Ñy item ─æß╗â mß╗ƒ reader")
                return False
            ...
            if self._extract_reader_payload(xml):
                return True
            texts = self.get_all_text(xml)
        if any(text.startswith("Ch╞░╞íng ") for text in texts):
            return True

        log_fn("  ΓÜá Kh├┤ng v├áo ─æ╞░ß╗úc reader")
        return False
    ...
    def open_reader_download_dialog(self, log_fn: Callable[[str], None] = print) -> bool:
        ...
        for attempt in range(2):
            ...
            if not self._reader_menu_visible(overlay_xml):
                continue
            center = self._find_clickable_text_center("Tß║úi truyß╗çn", exact=True, xml_str=overlay_xml)
            if center:
                break

        if not center:
            log_fn("  ΓÜá Kh├┤ng t├¼m thß║Ñy mß╗Ñc Tß║úi truyß╗çn trong reader")
            return False
        ...
        log_fn("  ΓÜá Kh├┤ng mß╗ƒ ─æ╞░ß╗úc popup tß║úi truyß╗çn")
        return False
    ...
    def submit_reader_download_range(
        ...
        dialog = self._parse_reader_download_dialog(self.dump_ui())
        if not dialog:
            log_fn("  ΓÜá Popup tß║úi truyß╗çn ch╞░a sß║╡n s├áng")
            return False
        ...
        if any("Bß║ín ─æ├ú tß║úi tß║Ñt cß║ú c├íc ch╞░╞íng rß╗ôi." in text for text in texts):
            log_fn(f"  App b├ío ─æ├ú tß║úi ─æß╗º ch{ch_start}-{ch_end}")
            return True
        if any("Bß║ín muß╗æn tß║úi" in text for text in texts):
            log_fn("  ΓÜá Popup tß║úi truyß╗çn vß║½n c├▓n mß╗ƒ sau khi bß║Ñm ─Éß╗ông ├╜")
            return False

        log_fn(f"  ─É├ú gß╗¡i lß╗çnh tß║úi trong app: ch{ch_start}-{ch_end}")
        return True
    ...
    def queue_current_book_full_download(
        ...
        if ch_end < ch_start:
            log_fn("  ΓÜá Khoß║úng ch╞░╞íng kh├┤ng hß╗úp lß╗ç")
            return False
    ...
    def return_to_library(self, log_fn: Callable[[str], None] = print, max_steps: int = 6) -> bool:
        ...
        log_fn("ΓÜá Kh├┤ng quay lß║íi ─æ╞░ß╗úc Tß╗º Truyß╗çn")
        return False
    ...
    def scan_visible_books(self, log_fn: Callable[[str], None] = print, xml_str: str = "") -> List[Dict]:
        ...
        try:
            ...
            log_fn("ΓÜá Kh├┤ng ─æß╗ìc ─æ╞░ß╗úc UI dump")
            return []
        ...
        books.sort(key=lambda item: (item["bounds"][1], item["bounds"][0]))
        if books:
            log_fn(f"Qu├⌐t thß║Ñy {len(books)} truyß╗çn tr├¬n m├án h├¼nh")
        return books
    ...
    def scan_book_list(self, max_items: int = 40, max_scrolls: int = 6,
        ...
        for round_idx in range(max_scrolls):
            ...
            log_fn(f"Qu├⌐t l╞░ß╗út {round_idx + 1}: +{new_count} truyß╗çn")
            if len(all_books) >= max_items:
                break
    ...
    # ΓöÇΓöÇ Text Extraction (speed-optimized) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    def read_current_chapter_payload(self,
        ...
        payload = self._extract_reader_payload(xml)
        if payload:
            log_fn(f"─Éß╗ìc nhanh {payload['title']}...")
            return payload

        collected, seen, repeats = [], set(), 0
        title = ""
        chapter_index = None
        log_fn("─Éß╗ìc nß╗Öi dung ch╞░╞íng...")

        for step in range(SCROLL_STEPS):
            dump = self.dump_ui()
            texts = self.get_all_text(dump)
            if not title:
                title = next((item for item in texts if item.startswith("Ch╞░╞íng ")), "")
                if title:
                    match = re.search(r"Ch╞░╞íng\s+(\d+)", title, re.IGNORECASE)
                    if match:
            ...
            if h_key in seen:
                repeats += 1
                if repeats >= 1:
                    log_fn(f"  Hß║┐t ch╞░╞íng (scroll {step})")
                    break
            else:
                seen.add(h_key)
                repeats = 0
            ...
            log_fn(f"  Scroll {step+1}: +{len(content)} ─æoß║ín")
            self.swipe_up()
