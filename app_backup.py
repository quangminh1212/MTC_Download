...
# Initialize SocketIO - commented out for now to avoid issues
...
def start_server():
    ...
    with open('server_startup.log', 'w', encoding='utf-8') as f:
    ...
    port = find_free_port()
    if port is None:
        msg = "❌ Không thể tìm thấy port khả dụng từ 5000-5100"
    ...
    # Log successful port finding
    with open('server_startup.log', 'a', encoding='utf-8') as f:
        f.write(f"Found port: {port}\n")
        f.flush()
    ...
    try:
        with open('server_startup.log', 'a', encoding='utf-8') as f:
            f.write("Starting SocketIO server...\n")
            f.flush()

        # Try to start server with different configurations
        # Use Flask built-in server for now
        ...
        error_msg = f"❌ Lỗi khởi động server: {e}"
